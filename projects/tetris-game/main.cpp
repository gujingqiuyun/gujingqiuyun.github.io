...
int main()
{
	...
	while (!bGameOver) // Main Loop
	{
		...
		// Input ========================
		// 使用更直观的按键控制
		bKey[0] = (GetAsyncKeyState(VK_RIGHT) & 0x8000) != 0; // 右
		bKey[1] = (GetAsyncKeyState(VK_LEFT) & 0x8000) != 0;  // 左
		bKey[2] = (GetAsyncKeyState(VK_DOWN) & 0x8000) != 0;  // 下
		bKey[3] = (GetAsyncKeyState(VK_UP) & 0x8000) != 0;    // 旋转
